...
export default function PatientHome({ d, go }: Props) {
  ...
  const tip = useMemo(() => CARE_TIPS[new Date().getDate() % CARE_TIPS.length], [])
          ...
          {nextMed ? (
            <>
              ...
              <p className="text-sm text-slate-500 mt-0.5">
                ...
                {nextMed.time_of_day && `, ${nextMed.time_of_day.toLowerCase()}`}
              </p>
              <button
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {([
              [Droplet, 'text-sky-500', 'Glucose',
               latestVital?.glucose_mmol != null ? `${latestVital.glucose_mmol}` : NONE, 'mmol/L'],
              [Heart, 'text-rose-500', 'Blood pressure',
               latestVital?.systolic && latestVital?.diastolic
                 ? `${latestVital.systolic}/${latestVital.diastolic}` : NONE, 'mmHg'],
              [Scale, 'text-teal-600', 'Weight',
               latestVital?.weight_kg != null ? `${latestVital.weight_kg}` : NONE, 'kg'],
        ...
        <section className={`${card} p-5`}>
          ...
          {alerts.length === 0 ? (
            <>
              ...
              <p className="text-sm text-slate-500 mt-0.5">You are doing well.</p>
            </>
          ) : (
